...
async function captureAndSendSnapshot() {
  try {
    ...
    const aiAnalysis = await networkTracker.classifyCurrentDomains(
      activeTab.url
    );
    ...
    const isAutoFlagged = AUTO_FLAGGED_DOMAINS.some(domain =>
    ...
    const VISION_WEIGHT = 0.60;  // Vision AI is powerful but not absolute
    const DOMAIN_WEIGHT = 0.40;  // Domain auto-flagging has significant weight
    const OFF_TASK_THRESHOLD = 0.50;  // 50% threshold - auto-flagged domains will trigger even if vision says on-task
    ...
    let screenshot = null;
    if (shouldRunVision) {
      console.log(`[Background] Running vision AI check (${isAutoFlagged ? 'auto-flagged domain' : `periodic check ${checkCounter}`})`);
      screenshot = await captureActiveTab();
    } else {
      console.log(`[Background] Skipping vision check (${checkCounter}) - using domain analysis only`);
    }
    if (screenshot) {
      const base64Image = dataUrlToBase64(screenshot);

      // Build context for vision AI
      const suspiciousReason = aiAnalysis.patterns.length > 0 || aiAnalysis.offTaskDomains.length > 0
        ? (aiAnalysis.patterns.join("; ") || `Off-task domains detected: ${aiAnalysis.offTaskDomains.join(", ")}`)
      ...
      if (isAutoFlagged) {
        ...
        console.log(`[Background] 🚩 AUTO-FLAGGED DOMAIN: ${currentDomain} - Always considered off-task`);
        ...
        domainConfidence = 0.60; // 60% - suspicious patterns
      }
  ...
  Final: OFF-TASK (95% confidence)`);
        ...
        const domainReason = aiAnalysis.offTaskDomains.length > 0
  ...
  Threshold: ${(OFF_TASK_THRESHOLD * 100).toFixed(0)}%`);
      ...
      if (isAutoFlagged) {
        ...
        enhancedSnapshot.state = "off_task";
        enhancedSnapshot.confidence = 0.90;
        ...
        console.log("[Background] Suspicious patterns but no vision - ON-TASK (waiting for vision check)");
      } else {
    ...
    if (lastPageUrl && lastPageStartTime) {
      ...
      const analyticsEntry: AnalyticsEntry = {
        ...
        duration,
        category: enhancedSnapshot.context?.visualVerification?.detectedContent?.split(",")[0]?.trim(),
      ...
      if (enhancedSnapshot.state === "off_task") {
        const reason = enhancedSnapshot.context?.visualVerification?.detectedContent ||
...
chrome.runtime.onMessage.addListener((message, sender, _sendResponse) => {
  ...
  if (message.type === "ALERT_TRIGGERED") {
    ...
    console.log(`[Background] TAB LOCKED: ${lockedTabId} - User cannot escape until puzzle is solved`);
  }
  ...
  if (message.type === "CLOSE_OFF_TASK_TAB") {
    ...
    chrome.tabs.query({ currentWindow: true }, (tabs) => {
      const onTaskDomains = [
        "github.com", "stackoverflow.com", "gitlab.com", "docs.", "developer.mozilla.org",
        "localhost", "127.0.0.1", ".edu"
      ];
